...
import OutboundPenerimaanLogHeader from "./OutboundPenerimaanLogHeader";
import OutboundPenerimaanLogItem from "./OutboundPenerimaanLogItem";
import { useOutboundPenerimaanLogStore } from "./store";
import { useOutboundPenerimaanLog } from "./useOutboundPenerimaanLog";
...
const COLUMN_ID_REFERENCE_NUMBER = "referenceNumber";
...
const OutboundPenerimaanLogPage = () => {
  const { t } = useTranslation("outbound");
  const {
    outboundPenerimaanLogData,
    isLoadingOutboundPenerimaanLogData,
    nextCursor,
    prevCursor,
    totalCount,
  } = useOutboundPenerimaanLog();
  const { tokenPayload } = useUser();

  const currentPage = useOutboundPenerimaanLogStore((state) => state.currentPage);
  const itemLimit = useOutboundPenerimaanLogStore((state) => state.itemLimit);

  const commonAttributes = useMemo(
    () => extractCommonAttributes(outboundPenerimaanLogData, tokenPayload?.organization_id),
    [outboundPenerimaanLogData, tokenPayload?.organization_id]
  );
  ...
  const allColumns = useMemo(() => {
    const staticColumns = [
      ...
      { id: COLUMN_ID_REFERENCE_NUMBER, label: "No. Ref" },
  ...
  const {
    ...
    hideAllColumns,
    visibleColumns,
  } = useColumnVisibility("outbound-penerimaan-log", allColumns);

  return (
    <div
      className={`flex w-full gap-4 flex-col ${
        outboundPenerimaanLogData.length === 0 ? "h-[calc(100vh-120px)]" : ""
      }`}
    >
      <div className="mt-4 flex-shrink-0">
        <OutboundPenerimaanLogHeader
      ...
      <div
        className={`w-full max-w-[91vw] lg:max-w-full flex-1 ${
          outboundPenerimaanLogData.length === 0 ? "overflow-visible" : "overflow-x-auto"
        }`}
      >
        {isInitialized && (
          <Table className="border shadow-md rounded-md">
            <TableHeader>
              <TableRow>
                ...
                {isColumnVisible(COLUMN_ID_REFERENCE_NUMBER) && (
                  <TableHead key={COLUMN_ID_REFERENCE_NUMBER}>No. Ref</TableHead>
                )}
            ...
            <TableBody>
              {!isLoadingOutboundPenerimaanLogData &&
                outboundPenerimaanLogData.length > 0 &&
                outboundPenerimaanLogData.map((item: StockMovementItem, index: number) => (
                  <OutboundPenerimaanLogItem
                    key={item.id}
...
export default OutboundPenerimaanLogPage;
