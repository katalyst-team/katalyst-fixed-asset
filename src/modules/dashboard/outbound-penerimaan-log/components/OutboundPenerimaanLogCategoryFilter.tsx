...
const OutboundPenerimaanLogCategoryFilter: React.FC = () => {
  const router = useRouter();
  const { tokenPayload } = useUser();
  const organizationId = tokenPayload?.organization_id ?? "";

  const { filters, resetPagination, setFilters } = useOutboundPenerimaanLogStore(
    useShallow((state) => ({
  ...
  const kayuBulatCategoryOptions = React.useMemo(() => {
  ...
  return (
    <MultiCombobox
      ...
      disabled={isLoading}
      emptyMessage="No KAYU BULAT categories available"
      label=""
      options={kayuBulatCategoryOptions}
...
export default OutboundPenerimaanLogCategoryFilter;
