...
import OutboundPenerimaanLogFilterContent from "./OutboundPenerimaanLogFilterContent";

const OutboundPenerimaanLogFilter: React.FC = () => {
  const { t } = useTranslation("outbound");
  const { tokenPayload, selectedTeam } = useUser();
  const organizationId = tokenPayload?.organization_id ?? "";

  const { currentFilters, resetPagination, setFilters } = useOutboundPenerimaanLogStore(
    useShallow((state) => ({
  ...
  return (
    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
      ...
      <PopoverContent align="end" className="w-[500px] p-0 flex flex-col max-h-[80vh]">
        ...
        <OutboundPenerimaanLogFilterContent
...
export default OutboundPenerimaanLogFilter;
