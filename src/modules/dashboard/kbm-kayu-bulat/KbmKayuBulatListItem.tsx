...
interface KbmKayuBulatListItemProps {
  attributeColumns: { id: string; name: string }[];
  item: CategoryItemType;
  num: number;
}

const KbmKayuBulatListItem = ({ attributeColumns, item, num }: KbmKayuBulatListItemProps) => {
  const { t } = useTranslation("kbm-kayu-bulat");
  const basePath = "/dashboard/kbm-kayu-bulat";
  ...
  const defaultsByAttrId = useMemo(() => {
    const map = new Map<string, string[]>();
    for (const d of item.attribute_defaults ?? []) {
      map.set(d.attribute.attribute.id, d.values);
    }
    return map;
  }, [item.attribute_defaults]);

  const defaultSubName = item.default_subcategory?.name ?? null;
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {defaultSubName ? (
          <Badge variant="secondary">{defaultSubName}</Badge>
        ) : (
          <span className="text-sm text-muted-foreground">-</span>
        )}
      </TableCell>
      {attributeColumns.map((col) => {
        const values = defaultsByAttrId.get(col.id);
        return (
      ...
      <TableCell className="text-center">
        <div className="flex justify-center gap-2">
          ...
          <KbmKayuBulatModalAdd
            categoryAttributeItems={editAttributeItems}
            categoryCode={item.code}
            categoryDefaults={editDefaults}
            categoryDefaultSubcategoryId={item.default_subcategory_id}
            categoryId={item.id}
...
export default KbmKayuBulatListItem;