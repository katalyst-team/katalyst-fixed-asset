...
const KAYU_BULAT_PREFIX = "KAYU BULAT ";

interface KbmKayuBulatModalAddProps {
  categoryAttributeItems?: CategoryAttributeItem[];
  categoryCode?: string | null;
  categoryDefaults?: AttributeDefaultRequest[];
  categoryDefaultSubcategoryId?: string | null;
  categoryId?: string;
  categoryName?: string;
...
const KbmKayuBulatModalAdd = ({
  categoryAttributeItems,
  categoryCode,
  categoryDefaults,
  categoryDefaultSubcategoryId,
  categoryId,
  categoryName,
  categoryStoreId,
  hasSubCategoryInitial,
  trigger,
}: KbmKayuBulatModalAddProps) => {
  ...
  const [defaultSubcategoryId, setDefaultSubcategoryId] = useState<string | null>(
    categoryDefaultSubcategoryId ?? null
  );
  ...
  useEffect(() => {
    if (subcategories.length > 0 && defaultSubcategoryId) {
      const exists = subcategories.some((s) => s.id === defaultSubcategoryId);
      if (!exists) {
        setDefaultSubcategoryId(null);
      }
    }
  }, [subcategories, defaultSubcategoryId]);
  ...
  const handleOpenChange = (newOpen: boolean) => {
    ...
    if (newOpen) {
      ...
      setDefaultSubcategoryId(categoryDefaultSubcategoryId ?? null);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!finalName.startsWith(KAYU_BULAT_PREFIX)) {
      finalName = `${KAYU_BULAT_PREFIX}${finalName}`;
    }
    ...
    if (hasSubCategory) {
      payload.default_subcategory_id = defaultSubcategoryId || undefined;
    }
      ...
      <DialogContent className="sm:max-w-[480px] max-h-[90vh] overflow-hidden">
        ...
        <ScrollArea className="max-h-[60vh] pr-2">
          <div className="flex flex-col gap-4 py-4 pr-2">
            ...
            {hasSubCategory && isEditMode && subcategories.length > 0 && (
              <div className="grid gap-2">
                <Label>{t("modal.add.defaultSubcategory")}</Label>
                <p className="text-xs text-muted-foreground">
                  {t("modal.add.defaultSubcategoryDescription")}
                </p>
                <Select
                  value={defaultSubcategoryId ?? "none"}
                  onValueChange={(val) => setDefaultSubcategoryId(val === "none" ? null : val)}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("modal.add.defaultSubcategoryPlaceholder")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">{t("modal.add.defaultSubcategoryNone")}</SelectItem>
...
export default KbmKayuBulatModalAdd;