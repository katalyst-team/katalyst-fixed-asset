...
interface KbmGradeImportModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const KbmGradeImportModal: React.FC<KbmGradeImportModalProps> = ({
  isOpen,
  onClose,
  autoCloseOnSuccess = false,
}) => {
  const { gradeType, translationNamespace } = useKbmGradeConfig();
  ...
  const typeAttributeId = useMemo(() => {
    if (!kbmGradeCategory?.attribute_items) return undefined;
    return kbmGradeCategory.attribute_items.find(
  ...
  const onDrop = useCallback(
      ...
      try {
        ...
        const parsed: ParsedRow[] = rows
          .map((row, index) => {
            ...
            if (!rowData[GRADE_NAME_COLUMN]) {
              errors.push(
                t("import.gradeNameRequired", "Grade Name is required")
              );
            }
        ...
        toast.error(
          t("import.parseError", "Failed to parse Excel file")
        );
        setFile(null);
      } finally {
    ...
    [exportableAttributes, kbmGradeCategory, t]
  );
  ...
  const handleImport = async () => {
    if (!parsedData.length || !kbmGradeCategory) return;
    if (!typeAttributeId) {
      toast.error(
        t(
          "import.typeAttributeMissing",
    ...
    for (let i = 0; i < parsedData.length; i++) {
      ...
      try {
        const rawName = row.data[GRADE_NAME_COLUMN] || "";
        const normalizedName = normalizeKbmGradeName(rawName);

        if (createdNames.has(normalizedName)) {
          results.failed++;
          results.errors.push({
            error: t("import.duplicateName", {
              defaultValue:
        ...
        let isDuplicate = duplicateCache.get(normalizedName);
        if (isDuplicate === undefined) {
          isDuplicate = await isDuplicateKbmGradeName({
            categoryId: kbmGradeCategory.id,
            gTypeAttributeId: typeAttributeId,
            gradeType,
            name: rawName,
            organizationId,
          });
          duplicateCache.set(normalizedName, isDuplicate);
        }
        ...
        const skuParams: CreateSkuParams = {
          attribute_items,
          category_ids: [kbmGradeCategory.id],
          image_urls: [],
          name: row.data[GRADE_NAME_COLUMN],
    ...
    if (results.success > 0) {
      ...
      toast.success(
        t("import.successMessage", {
          ...
          defaultValue: `Successfully imported ${results.success} grade(s)`,
        })
      );
      ...
      toastError(
        new Error(
          t("import.failedMessage", {
            ...
            defaultValue: `Failed to import ${results.failed} grade(s)`,
          })
        )
      );
    }
      ...
      <DialogContent className="max-w-4xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t("import.title", "Import Grades from Excel")}
          </DialogTitle>
          <DialogDescription>
            {t(
              "import.description",
              "Upload an Excel file to bulk create grades. Use the template for correct format."
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <ScrollArea className="max-h-[60vh]">
          <div className="space-y-4 py-4 pr-4">
            <Alert>
              ...
              <AlertDescription className="text-sm space-y-2">
                ...
                <ol className="list-decimal list-inside space-y-1 ml-2">
                  ...
                  <li>
                    {t("import.step2", "Fill in your grade data")}
                  </li>
                  <li>
                    {t(
                      "import.step3",
                ...
                <p className="text-xs text-muted-foreground">
                  {t("import.note", {
                    defaultValue:
                      "G_TYPE is auto-set to {{gradeType}} for every row.",
                    gradeType,
                  })}
                </p>
              </AlertDescription>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t("import.storeHint", "Imported grades will be assigned to the selected store")}
              </p>
            </div>
            ...
            {isProcessing && !importResult && (
              <div className="space-y-2">
                <p className="text-sm font-medium">
                  {importProgress > 0
                    ? t("import.importing", "Importing grades...")
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>{GRADE_NAME_COLUMN}</TableHead>
                          <TableHead>
            ...
            >
              {isProcessing
                ...
                : t("import.importButton", {
                    ...
                    defaultValue: `Import ${validRows.length} grade(s)`,
                  })}
            </Button>
          )}
        </DialogFooter>
