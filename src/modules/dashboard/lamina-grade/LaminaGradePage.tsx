...
const LaminaGradePage = () => {
  ...
  const laminaCategories = useMemo(() => {
    return (categoryData?.data?.categories ?? []).filter((c) =>
      c.name.startsWith("LAMINA"),
    );
  }, [categoryData]);

  const selectedCategory = useMemo(
    () => laminaCategories.find((c) => c.id === selectedCategoryId),
    [laminaCategories, selectedCategoryId],
  );
  ...
  useEffect(() => {
    if (!selectedCategoryId && laminaCategories.length > 0) {
      setSelectedCategoryId(laminaCategories[0].id);
    }
  }, [laminaCategories, selectedCategoryId]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="mt-4 flex items-center justify-between gap-3 flex-nowrap">
...
export default LaminaGradePage;
