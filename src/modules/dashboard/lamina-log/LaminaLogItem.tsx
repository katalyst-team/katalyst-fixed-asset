...
const LaminaLogItem: React.FC<LaminaLogItemProps> = ({
  isColumnVisible,
  item,
  num,
  uniqueAttributes,
  ...
  const { BadgeComponent: itemStatusBadge } = useBadgeStatus(
    item.item?.status?.name || "",
    {
      ...
      translationNamespace: "lamina-log",
    }
  );
              ...
              <div className="flex -space-x-2">
                {imageUrls.slice(0, 3).map((imageUrl, imgIndex) => (
                  ...
                  >
                    <Image
                      fill
                      alt={t("lamina-log:item.imageAlt", {
