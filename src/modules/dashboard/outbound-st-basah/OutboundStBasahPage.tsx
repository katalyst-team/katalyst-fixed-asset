...
import OutboundStBasahHeader from "./OutboundStBasahHeader";
import OutboundStBasahItem from "./OutboundStBasahItem";
import { useOutboundStBasahStore } from "./store";
import { useOutboundStBasah } from "./useOutboundStBasah";
...
const OutboundStBasah = () => {
  ...
  const currentPage = useOutboundStBasahStore((state) => state.currentPage);
  const itemLimit = useOutboundStBasahStore((state) => state.itemLimit);

  const commonAttributes = useMemo(
    () => extractCommonAttributes(outboundStBasahData, tokenPayload?.organization_id),
    [outboundStBasahData, tokenPayload?.organization_id]
  );
  ...
  const {
    ...
    hideAllColumns,
    visibleColumns,
  } = useColumnVisibility("outbound-st-basah", allColumns);

  return (
    <div
      className={`flex w-full gap-4 flex-col ${
        outboundStBasahData.length === 0 ? "h-[calc(100vh-120px)]" : ""
      }`}
    >
      <div className="mt-4 flex-shrink-0">
        <OutboundStBasahHeader
      ...
      <div
        className={`w-full max-w-[91vw] lg:max-w-full flex-1 ${
          outboundStBasahData.length === 0 ? "overflow-visible" : "overflow-x-auto"
        }`}
      >
        {isInitialized && (
          <Table className="border shadow-md rounded-md">
            ...
            <TableBody>
              {!isLoadingOutboundStBasahData &&
                outboundStBasahData.length > 0 &&
                outboundStBasahData.map((item: StockMovementItem, index: number) => (
                  <OutboundStBasahItem
                    key={item.id}
...
export default OutboundStBasah;
