...
interface StPenerimaanLogLogItemProps {
  item: SkuItemType;
...
const StPenerimaanLogLogItem: React.FC<StPenerimaanLogLogItemProps> = ({
  isColumnVisible,
  item,
  num,
  uniqueAttributes,
  ...
  const { BadgeComponent: itemStatusBadge } = useBadgeStatus(
    item.item?.status?.name || "",
    {
      ...
      translationNamespace: "st-penerimaan-log-log",
    }
  );
              ...
              <div className="flex -space-x-2">
                {imageUrls.slice(0, 3).map((imageUrl, imgIndex) => (
                  ...
                  >
                    <Image
                      fill
                      alt={t("st-penerimaan-log-log:item.imageAlt", {
...
export default StPenerimaanLogLogItem;
