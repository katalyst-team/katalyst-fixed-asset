...
import StPenerimaanLogLogHeader from "./StPenerimaanLogLogHeader";
import StPenerimaanLogLogItem from "./StPenerimaanLogLogItem";
import { useStPenerimaanLogLog } from "./useStPenerimaanLogLog";

// Column IDs for st-penerimaan-log-log
...
const StPenerimaanLogLog = () => {
  const { t } = useTranslation(["common", "st-penerimaan-log-log"]);
  const {
    stPenerimaanLogLogData,
    isLoadingStPenerimaanLogLogData,
    nextCursor,
    prevCursor,
    totalCount,
  } = useStPenerimaanLogLog();
  const { tokenPayload } = useUser();

  // Use Zustand store - get primitive values to avoid re-render loops
  const currentPage = useStPenerimaanLogLogStore((state) => state.currentPage);
  const itemLimit = useStPenerimaanLogLogStore((state) => state.itemLimit);
  const filters = useStPenerimaanLogLogStore(
  ...
  const uniqueAttributes = useMemo(() => {
    return extractUniqueAttributes(stPenerimaanLogLogData, organizationId);
  }, [stPenerimaanLogLogData, organizationId]);
  ...
  const allColumns = useMemo(() => {
    const staticColumns = [
      { id: COLUMN_ID_NO, label: t("st-penerimaan-log-log:table.header.no", "No") },
      { id: COLUMN_ID_IMAGE, label: t("st-penerimaan-log-log:table.header.image", "Image") },
      { id: COLUMN_ID_NAME, label: t("st-penerimaan-log-log:table.header.name", "Name") },
      {
        id: COLUMN_ID_INTERNAL_CODE,
        label: t("st-penerimaan-log-log:table.header.internalCode", "Internal Code"),
      },
      { id: COLUMN_ID_RFID_EPC, label: t("st-penerimaan-log-log:table.header.rfidEpc", "RFID EPC") },
      { id: COLUMN_ID_RFID_NAME, label: t("st-penerimaan-log-log:table.header.rfidName", "RFID Name") },
      { id: COLUMN_ID_CATEGORY, label: t("st-penerimaan-log-log:table.header.category", "Category") },
      { id: COLUMN_ID_STATUS, label: t("st-penerimaan-log-log:table.header.status", "Status") },
      {
        id: COLUMN_ID_ITEM_STATUS,
        label: t("st-penerimaan-log-log:table.header.itemStatus", "Item Status"),
      },
      {
        id: COLUMN_ID_STOCK_MOVEMENT_TYPE,
        label: t("st-penerimaan-log-log:table.header.stockMovementType", "Stock Movement Type"),
      },
    ];
  ...
  const {
    ...
    hideAllColumns,
    visibleColumns,
  } = useColumnVisibility("st-penerimaan-log-log", allColumns);

  return (
    <div
      className={`flex w-full gap-4 flex-col ${
        stPenerimaanLogLogData.length === 0 ? "h-[calc(100vh-120px)]" : ""
      }`}
    >
      <div className="mt-4 flex-shrink-0">
        <StPenerimaanLogLogHeader
      ...
      <div
        className={`w-full max-w-[91vw] lg:max-w-full flex-1 ${
          stPenerimaanLogLogData.length === 0 ? "overflow-visible" : "overflow-x-auto"
        }`}
      >
        {isInitialized && (
          <Table className="border shadow-md rounded-md">
            <TableHeader>
              <TableRow>
                {isColumnVisible(COLUMN_ID_NO) && (
                  <TableHead>{t("st-penerimaan-log-log:table.header.no", "No")}</TableHead>
                )}
                {isColumnVisible(COLUMN_ID_IMAGE) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.image", "Image")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_NAME) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.name", "Name")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_INTERNAL_CODE) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.internalCode", "Internal Code")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_RFID_EPC) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.rfidEpc", "RFID EPC")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_RFID_NAME) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.rfidName", "RFID Name")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_CATEGORY) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.category", "Category")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_STATUS) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.status", "Status")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_ITEM_STATUS) && (
                  <TableHead>
                    {t("st-penerimaan-log-log:table.header.itemStatus", "Item Status")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_STOCK_MOVEMENT_TYPE) && (
                  <TableHead>
                    {t(
                      "st-penerimaan-log-log:table.header.stockMovementType",
                      "Stock Movement Type",
                    )}
                  </TableHead>
                )}
            ...
            <TableBody>
              {!isLoadingStPenerimaanLogLogData &&
                stPenerimaanLogLogData.length > 0 &&
                stPenerimaanLogLogData.map((item: SkuItemType, index: number) => (
                  <StPenerimaanLogLogItem
                    key={item.id}
        ...
        {isLoadingStPenerimaanLogLogData ? (
          ...
          stPenerimaanLogLogData.length === 0 && (
            <EmptyState
              className="mt-4"
              description={t(
                "st-penerimaan-log-log:empty.description",
                "No ST Penerimaan Log log entries found.",
              )}
              title={t("st-penerimaan-log-log:empty.title", "No ST Penerimaan Log Log Entries")}
            />
          )
        )}
      </div>
...
export default StPenerimaanLogLog;
