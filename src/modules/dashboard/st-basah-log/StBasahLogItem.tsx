...
const StBasahLogItem: React.FC<StBasahLogItemProps> = ({
  isColumnVisible,
  item,
  num,
  uniqueAttributes,
  ...
  const { BadgeComponent: itemStatusBadge } = useBadgeStatus(
    item.item?.status?.name || "",
    {
      ...
      translationNamespace: "st-basah-log",
    }
  );
              ...
              <div className="flex -space-x-2">
                {imageUrls.slice(0, 3).map((imageUrl, imgIndex) => (
                  ...
                  >
                    <Image
                      fill
                      alt={t("st-basah-log:item.imageAlt", {
