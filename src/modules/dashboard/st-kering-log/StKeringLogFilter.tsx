...
const StKeringLogFilter: React.FC = () => {
  const { t } = useTranslation(["common", "st-kering-log"]);
  const router = useRouter();
  const { tokenPayload } = useUser();
  const organizationId = tokenPayload?.organization_id ?? "";

  const { currentFilters, resetPagination, setFilters } = useStKeringLogStore(
    useShallow((state) => ({
    ...
    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
      <PopoverTrigger asChild>
        <Button size="sm" variant="outline">
          ...
          {t("st-kering-log:filter.button", "Filter")}
          {hasActiveFilters && (
      ...
      >
        <div className="p-4 border-b bg-background">
          <h2 className="font-semibold">
            {t("st-kering-log:filter.title", "Filter Options")}
          </h2>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <Combobox
            label={t("st-kering-log:filter.timeframe", "Time Range")}
            options={timeframeOptions}
            placeholder={t(
              "st-kering-log:filter.timeframePlaceholder",
              "Select time range...",
            )}
            value={timeframe}
          ...
          <div className="space-y-2">
            <Label className="text-sm font-medium">
              {t("st-kering-log:filter.startDate", "Start Date")}
            </Label>
            <DatePicker
              buttonClassName="w-full"
              className="w-full"
              id="stKeringStartDate"
              placeholder={t("st-kering-log:filter.startDate", "Start Date")}
          ...
          <div className="space-y-2">
            <Label className="text-sm font-medium">
              {t("st-kering-log:filter.endDate", "End Date")}
            </Label>
            <DatePicker
              buttonClassName="w-full"
              className="w-full"
              id="stKeringEndDate"
              placeholder={t("st-kering-log:filter.endDate", "End Date")}
              value={endDateValue}
          ...
          <div className="space-y-2 border-t pt-4">
            <Label className="text-sm font-medium">
              {t(
                "st-kering-log:filter.dateAttributes.title",
            ...
            {isLoadingAttributes ? (
                            ...
                            <div className="space-y-2">
                              <div>
                                <Label className="text-xs">
                                  {t(
                                    "st-kering-log:filter.dateAttributes.startDate",
                                    "Start Date",
                                  )}
                                </Label>
                                <Collapsible>
                              ...
                              <div>
                                <Label className="text-xs">
                                  {t(
                                    "st-kering-log:filter.dateAttributes.endDate",
                                    "End Date",
                                  )}
                                </Label>
                                <Collapsible>
              ...
              <div className="text-sm text-muted-foreground">
                {t(
                  "st-kering-log:filter.noDateAttributes",
          ...
          {selectableAttributes.length > 0 && (
            <div className="space-y-2 border-t pt-4">
              <Label className="text-sm font-medium">
                {t(
                  "st-kering-log:filter.textAttributes.title",
                ...
                {selectableAttributes.map((attr) => (
                  <div key={attr.id} className="space-y-2">
                    ...
                    {isPresetAttribute(attr) ? (
                      ...
                      <Input
                        placeholder={t(
                          "st-kering-log:filter.textAttributes.placeholder",
                          "Enter value...",
                        )}
...
export default StKeringLogFilter;
