...
import SusunRstModal from "./components/SusunRstModal";
import StKeringLogHeader from "./StKeringLogHeader";
import StKeringLogItem from "./StKeringLogItem";
import { useStKeringLogStore } from "./store";
import { useStKeringLog } from "./useStKeringLog";
...
const StKeringLog = () => {
  ...
  const currentPage = useStKeringLogStore((state) => state.currentPage);
  ...
  const [assignAreaItem, setAssignAreaItem] = useState<SkuItemType | null>(
    null,
  );
  const [susunRstItem, setSusunRstItem] = useState<SkuItemType | null>(null);
  ...
  const uniqueAttributes = useMemo(() => {
    return extractUniqueAttributes(stKeringLogData, organizationId);
  }, [stKeringLogData, organizationId]);
  ...
  const allColumns = useMemo(() => {
    const staticColumns = [
      { id: COLUMN_ID_NO, label: t("st-kering-log:table.header.no", "No") },
      { id: COLUMN_ID_IMAGE, label: t("st-kering-log:table.header.image", "Image") },
      { id: COLUMN_ID_NAME, label: t("st-kering-log:table.header.name", "Name") },
      {
        id: COLUMN_ID_INTERNAL_CODE,
        label: t("st-kering-log:table.header.internalCode", "Internal Code"),
      },
      { id: COLUMN_ID_RFID_EPC, label: t("st-kering-log:table.header.rfidEpc", "RFID EPC") },
      { id: COLUMN_ID_RFID_NAME, label: t("st-kering-log:table.header.rfidName", "RFID Name") },
      { id: COLUMN_ID_CATEGORY, label: t("st-kering-log:table.header.category", "Category") },
      { id: COLUMN_ID_STATUS, label: t("st-kering-log:table.header.status", "Status") },
      {
        id: COLUMN_ID_ITEM_STATUS,
        label: t("st-kering-log:table.header.itemStatus", "Item Status"),
      },
      {
        id: COLUMN_ID_STOCK_MOVEMENT_TYPE,
        label: t("st-kering-log:table.header.stockMovementType", "Stock Movement Type"),
      },
    ];
  ...
  const {
    ...
    hideAllColumns,
    visibleColumns,
  } = useColumnVisibility("st-kering-log", allColumns);

  return (
    <div
      className={`flex w-full gap-4 flex-col ${
        stKeringLogData.length === 0 ? "h-[calc(100vh-120px)]" : ""
      }`}
    >
      ...
      <div
        className={`w-full max-w-[91vw] lg:max-w-full flex-1 ${
          stKeringLogData.length === 0 ? "overflow-visible" : "overflow-x-auto"
        }`}
      >
        {isInitialized && (
          <Table className="border shadow-md rounded-md">
            <TableHeader>
              <TableRow>
                {isColumnVisible(COLUMN_ID_NO) && (
                  <TableHead>{t("st-kering-log:table.header.no", "No")}</TableHead>
                )}
                {isColumnVisible(COLUMN_ID_IMAGE) && (
                  <TableHead>
                    {t("st-kering-log:table.header.image", "Image")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_NAME) && (
                  <TableHead>
                    {t("st-kering-log:table.header.name", "Name")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_INTERNAL_CODE) && (
                  <TableHead>
                    {t("st-kering-log:table.header.internalCode", "Internal Code")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_RFID_EPC) && (
                  <TableHead>
                    {t("st-kering-log:table.header.rfidEpc", "RFID EPC")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_RFID_NAME) && (
                  <TableHead>
                    {t("st-kering-log:table.header.rfidName", "RFID Name")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_CATEGORY) && (
                  <TableHead>
                    {t("st-kering-log:table.header.category", "Category")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_STATUS) && (
                  <TableHead>
                    {t("st-kering-log:table.header.status", "Status")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_ITEM_STATUS) && (
                  <TableHead>
                    {t("st-kering-log:table.header.itemStatus", "Item Status")}
                  </TableHead>
                )}
                {isColumnVisible(COLUMN_ID_STOCK_MOVEMENT_TYPE) && (
                  <TableHead>
                    {t(
                      "st-kering-log:table.header.stockMovementType",
                      "Stock Movement Type",
                    )}
                  </TableHead>
                )}
            ...
            <TableBody>
              {!isLoadingStKeringLogData &&
                stKeringLogData.length > 0 &&
                stKeringLogData.map((item: SkuItemType, index: number) => (
                  <StKeringLogItem
                    key={item.id}
        ...
        {isLoadingStKeringLogData ? (
          ...
          stKeringLogData.length === 0 && (
            <EmptyState
              className="mt-4"
              description={t(
                "st-kering-log:empty.description",
                "No ST Kering log entries found.",
              )}
              title={t("st-kering-log:empty.title", "No ST Kering Log Entries")}
            />
          )
        )}
      </div>
      ...
      <AssignAreaModal
        item={assignAreaItem}
        open={assignAreaItem !== null}
        onClose={() => setAssignAreaItem(null)}
      />
      <SusunRstModal
        item={susunRstItem}
        open={susunRstItem !== null}
        tanggalPotongAttrId={tanggalPotongAttrId}
        onClose={() => setSusunRstItem(null)}
      />
    </div>
  );
};

export default StKeringLog;
