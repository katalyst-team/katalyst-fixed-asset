...
interface UseOutboundLogReturn {
...
export const useOutboundLog = (): UseOutboundLogReturn => {
  const { tokenPayload } = useUser();

  const itemLimit = useOutboundLogStore((state) => state.itemLimit);
  const filters = useOutboundLogStore(useShallow((state) => state.filters));
  const selectedStoreId = useOutboundLogStore((state) => state.selectedStoreId);
  ...
  const outboundLogTypeIds = useMemo(() => {
    return (
      stockMovementTypesData
        ?.filter((type: StockMovementType) => type.name === StockMovementTypeNameEnum.ST_KERING_OUTBOUND)
  ...
  const requestFilters = useMemo(
    ...
    [filters, outboundLogTypeIds, itemLimit, stKeringCategoryIds],
  );
