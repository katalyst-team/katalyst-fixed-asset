...
const LaminaOutboundLogCategoryFilter: React.FC = () => {
  const router = useRouter();
  const { tokenPayload } = useUser();
  const organizationId = tokenPayload?.organization_id ?? "";

  const { filters, resetPagination, setFilters } = useLaminaOutboundLogStore(
    useShallow((state) => ({
...
export default LaminaOutboundLogCategoryFilter;
