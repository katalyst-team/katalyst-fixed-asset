...
import InboundPenerimaanLogHeader from "./InboundPenerimaanLogHeader";
import InboundPenerimaanLogItem from "./InboundPenerimaanLogItem";
import { useInboundPenerimaanLogStore } from "./store";
import { useInboundPenerimaanLog } from "./useInboundPenerimaanLog";
...
const InboundPenerimaanLog = () => {
  const { t } = useTranslation("inbound");
  const {
    inboundPenerimaanLogData,
    isLoadingInboundPenerimaanLogData,
    nextCursor,
    prevCursor,
    totalCount,
  } = useInboundPenerimaanLog();
  const { tokenPayload } = useUser();

  const currentPage = useInboundPenerimaanLogStore((state) => state.currentPage);
  const itemLimit = useInboundPenerimaanLogStore((state) => state.itemLimit);

  const commonAttributes = useMemo(
    () =>
      extractCommonAttributes(inboundPenerimaanLogData, tokenPayload?.organization_id),
    [inboundPenerimaanLogData, tokenPayload?.organization_id],
  );
  ...
  const allColumns = useMemo(() => {
    const staticColumns = [
      ...
      { id: COLUMN_ID_REFERENCE_NUMBER, label: "No. RIT" },
  ...
  const {
    ...
    hideAllColumns,
    visibleColumns,
  } = useColumnVisibility("inbound-penerimaan-log", allColumns);

  return (
    <div
      className={`flex w-full gap-4 flex-col ${
        inboundPenerimaanLogData.length === 0 ? "h-[calc(100vh-120px)]" : ""
      }`}
    >
      <div className="mt-4 flex-shrink-0">
        <InboundPenerimaanLogHeader
      ...
      <div
        className={`w-full max-w-[91vw] lg:max-w-full flex-1 ${
          inboundPenerimaanLogData.length === 0 ? "overflow-visible" : "overflow-x-auto"
        }`}
      >
        {isInitialized && (
          <Table className="border shadow-md rounded-md">
            <TableHeader>
              <TableRow>
                ...
                {isColumnVisible(COLUMN_ID_REFERENCE_NUMBER) && (
                  <TableHead key={COLUMN_ID_REFERENCE_NUMBER}>No. RIT</TableHead>
                )}
            ...
            <TableBody>
              {!isLoadingInboundPenerimaanLogData &&
                inboundPenerimaanLogData.length > 0 &&
                inboundPenerimaanLogData.map((item: StockMovementItem, index: number) => (
                  <InboundPenerimaanLogItem
                    key={item.id}
...
export default InboundPenerimaanLog;
