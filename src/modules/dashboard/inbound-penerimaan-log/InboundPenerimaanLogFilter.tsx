...
const InboundPenerimaanLogFilter: React.FC = () => {
  const { t } = useTranslation("inbound");
  const { tokenPayload, selectedTeam } = useUser();
  const organizationId = tokenPayload?.organization_id ?? "";

  const { currentFilters, resetPagination, setFilters } = useInboundPenerimaanLogStore(
    useShallow((state) => ({
  ...
  const getFiltersKey = React.useCallback(
    (filters: InboundFilterOptions) => JSON.stringify(filters ?? {}),
    []
  );
  ...
  const fromQuery = React.useCallback(
    (q: Record<string, string | string[] | undefined>): Partial<InboundFilterOptions> => ({
  ...
  const toQuery = React.useCallback(
    (filters: InboundFilterOptions): Record<string, string | string[] | undefined> => {
  ...
  const onInit = React.useCallback(
    (urlFilters: Partial<InboundFilterOptions>) => {
  ...
  const { syncToUrl } = useUrlFilterSync<InboundFilterOptions>({
    fromQuery,
    onInit,
    toQuery,
  });
  ...
  const handleReset = () => {
    ...
    handleSetFilters({});
    syncToUrl({} as InboundFilterOptions);
    setIsPopoverOpen(false);
  };
...
export default InboundPenerimaanLogFilter;
