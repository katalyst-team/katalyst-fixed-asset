...
const StBasahGradePage = () => {
  ...
  const stBasahCategories = useMemo(() => {
    return (categoryData?.data?.categories ?? []).filter((c) =>
      c.name.includes("BASAH"),
    );
  }, [categoryData]);

  useEffect(() => {
    if (!selectedCategoryId && stBasahCategories.length > 0) {
      setSelectedCategoryId(stBasahCategories[0].id);
    }
  }, [stBasahCategories, selectedCategoryId]);
...
export default StBasahGradePage;
