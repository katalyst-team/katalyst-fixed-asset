...
import LaminaInboundLogHeader from "./LaminaInboundLogHeader";
import LaminaInboundLogItem from "./LaminaInboundLogItem";
import { useLaminaInboundLogStore } from "./store";
import { useLaminaInboundLog } from "./useLaminaInboundLog";
...
const LaminaInboundLog = () => {
  const { t } = useTranslation("inbound");
  const {
    laminaInboundLogData,
  ...
  const currentPage = useLaminaInboundLogStore((state) => state.currentPage);
  const itemLimit = useLaminaInboundLogStore((state) => state.itemLimit);

  const commonAttributes = useMemo(
    () =>
      extractCommonAttributes(laminaInboundLogData, tokenPayload?.organization_id),
    [laminaInboundLogData, tokenPayload?.organization_id],
  );
  ...
  const {
    ...
    hideAllColumns,
    visibleColumns,
  } = useColumnVisibility("lamina-inbound-log", allColumns);

  return (
    <div
      className={`flex w-full gap-4 flex-col ${
        laminaInboundLogData.length === 0 ? "h-[calc(100vh-120px)]" : ""
      }`}
    >
      <div className="mt-4 flex-shrink-0">
        <LaminaInboundLogHeader
      ...
      <div
        className={`w-full max-w-[91vw] lg:max-w-full flex-1 ${
          laminaInboundLogData.length === 0 ? "overflow-visible" : "overflow-x-auto"
        }`}
      >
        {isInitialized && (
          <Table className="border shadow-md rounded-md">
            ...
            <TableBody>
              {!isLoadingLaminaInboundLogData &&
                laminaInboundLogData.length > 0 &&
                laminaInboundLogData.map((item: StockMovementItem, index: number) => (
                  <LaminaInboundLogItem
                    key={item.id}
...
export default LaminaInboundLog;
