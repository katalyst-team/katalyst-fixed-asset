...
import DetailSkuItem from "./DetailSkuItem";
import { useDetailSku } from "./useDetailSku";
...
const DetailSku = () => {
  ...
  return (
    <div className="flex w-full gap-4 flex-col">
      <h1 className="text-2xl font-bold font-heading">{t("detail.sku")}</h1>
      <section className="shadow-sm rounded-xl border border-border overflow-hidden">
        ...
        <Table>
          ...
          <TableBody>
            {detailSkuData.map((item) => (
              <DetailSkuItem key={item.no} item={item} />
            ))}
          </TableBody>
        </Table>
      </section>
