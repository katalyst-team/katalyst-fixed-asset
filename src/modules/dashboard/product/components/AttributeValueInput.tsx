...
export const AttributeValueInput = ({
  ...
  const refItems = refItemsData?.data?.items ?? [];
  ...
  switch (attribute.type) {
    case AttributeTypeEnum.SELECT:
      return (
        <Select disabled={disabled} value={selectedOption} onValueChange={handleSelectChange}>
          ...
          <SelectContent>
            ...
            {refItems.length === 0 && (
              <SelectItem disabled value="__empty__">
                {t("modal.addSku.noReferenceItems", "No reference items found")}
              </SelectItem>
            )}
          </SelectContent>
        </Select>
