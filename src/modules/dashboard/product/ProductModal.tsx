...
/**
 * Modal component for adding or editing a unique product with attributes
 */
export const ProductModal = ({ trigger, sku }: ProductModalProps) => {
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      if (isEditMode && sku) {
        ...
        toast.success(t("modal.addSku.updateSuccess"));
      } else {
        // Create new SKU
        await createSkuData({
          attribute_items,
        ...
        toast.success(t("modal.addSku.createSuccess"));
      }
      ...
      <DialogContent className="max-w-[425px] max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="p-6 pb-0 shrink-0">
          <DialogTitle className="text-xl">
            {isEditMode ? t("modal.addSku.editTitle") : t("modal.addSku.title")}
          </DialogTitle>
          <DialogDescription>
            {isEditMode
              ? t("modal.addSku.editDescription")
              : t("modal.addSku.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <div className="flex-1 overflow-y-auto px-6">
            <div className="grid gap-4 py-4">
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue
                          placeholder={t("modal.addSku.selectCategory")}
                        />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <ImageUploadWithCamera
                description={t("modal.addSku.imageDescription")}
                disabled={isDisabled}
                featureId="product"
                label={t("modal.addSku.imageLabel")}
                maxImages={5}
                prefix="sku-image"
              ...
              {selectedCategoryId && categoryAttributes.length > 0 && (
                <div className="grid gap-4">
                  <Label>{t("modal.addSku.attributesLabel")}</Label>
                  {isCategoriesLoading ? (
                    <div className="text-sm text-muted-foreground">
                      {t("modal.addSku.loadingAttributes")}
                    </div>
                  ) : (
          ...
          <DialogFooter className="px-6 py-4 border-t shrink-0">
            <Button disabled={isProcessing} type="submit">
              {isProcessing
                ? isEditMode
                  ? t("modal.addSku.updating")
                  : t("modal.addSku.creating")
                : isEditMode
                  ? t("modal.addSku.update")
                  : t("modal.addSku.create")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
