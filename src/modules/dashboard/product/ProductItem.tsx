  ...
  const handleDeleteSku = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    deleteSku(
      ...
      },
      {
        ...
        onSuccess: () => {
          toast.success(
            t("product:modal.addProduct.productDeleted", {
              defaultValue: t(
                "sku:modal.addSku.skuDeleted",
    ...
    <TableRow>
              ...
              <AccordionTrigger className="hover:no-underline">
                <div className="flex items-center gap-2">
                  <span className="text-sm">
                    {t("product:table.attributesLabel", {
      ...
      <TableCell>
        <div className="flex h-full gap-2">
          {canUpdate && (
            <ButtonEdit
              onClick={() => router.push(`/dashboard/product/edit/${item.id}`)}
            />
          )}
          {canDelete && <ButtonDelete onSubmit={handleDeleteSku} />}
          <ButtonDetail href={`/dashboard/product/${item.id}`} />
        </div>
      </TableCell>
    </TableRow>
  );
