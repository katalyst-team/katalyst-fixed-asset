...
interface KbmKayuStKeringListItemProps {
  item: CategoryItemType;
  num: number;
}

const KbmKayuStKeringListItem = ({ item, num }: KbmKayuStKeringListItemProps) => {
  const { t } = useTranslation("kbm-kayu-st-kering");
  const basePath = "/dashboard/kbm-kayu-st-kering";
  ...
  const editAttributeItems = hasDirectAttributes
    ? item.attribute_items!.map((ai) => ({
        attribute_id: ai.attribute.id,
        is_required: ai.is_required,
      }))
    : [];
  const editDefaults = hasDirectAttributes
...
export default KbmKayuStKeringListItem;
